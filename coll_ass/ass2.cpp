// #include<iostream>
// using namespace std;
// class Product{
//     int prod_id,prod_price,prod_quantity;
//     string prod_name;
//     static int prod_num;
// public:
//     Product(int prod_id,string prod_name,int prod_price,int prod_quantity){
//         this->prod_id=prod_id;
//         this->prod_name=prod_name;
//         this->prod_price=prod_price;
//         this->prod_quantity=prod_quantity;
//         prod_num++;
// }
//     void display(){
//         cout<<endl<<"Product ID:"<<prod_id<<"\nProduct Name:"<<prod_name<<"\nProduct Price"<<prod_price<<"\nProduct quantity"<<prod_quantity<<endl;
//     }
//     static void count(){
//         cout<<prod_num<<endl;
//     }
//     inline int stock(){
//         return prod_price*prod_quantity;
//     }
//     friend void compare(Product p1,Product p2);
// };

// void compare(Product p1,Product p2){
//     if (p1.prod_price>p2.prod_price)
//         cout<<p1.prod_name<<" is more expensive than "<<p2.prod_name<<endl;
    
//     else if (p1.prod_price<p2.prod_price)
//         cout<<p2.prod_name<<" is more expensive than "<<p1.prod_name<<endl;
    
//     else
//         cout<<"Both the product are of same price"<<endl;
    
// }

// int Product::prod_num=0;
// int main (){
// Product p1(1,"AC",50000,20);
// Product p2(2,"Table",1000,100);
// p1.display();
// p2.display();
// compare(p1,p2);
// Product::count();
// return 0;
// }




#include <iostream>
#include <string>
using namespace std;

class Product {
private:
    int productID;
    string productName;
    double productPrice;
    int quantity;

    static int totalProducts;

public:
    Product(int id = 0, string name = "", double price = 0.0, int qty = 0) {
        productID = id;
        productName = name;
        productPrice = price;
        quantity = qty;
        totalProducts++;
    }

    ~Product() {
        totalProducts--;
    }

    void inputDetails() {
        cout << "Enter Product ID: ";
        cin >> productID;
        cin.ignore(); 
        cout << "Enter Product Name: ";
        getline(cin, productName);
        cout << "Enter Product Price: ";
        cin >> productPrice;
        cout << "Enter Quantity: ";
        cin >> quantity;
    }

    void displayDetails() const {
        cout << "\nProduct ID: " << productID;
        cout << "\nProduct Name: " << productName;
        cout << "\nProduct Price: " << productPrice;
        cout << "\nQuantity: " << quantity;
        cout << "\nTotal Stock Value: " << stockValue() << endl;
    }

    inline double stockValue() const {
        return productPrice * quantity;
    }

    static void displayTotalProducts() {
        cout << "\nTotal Products Created: " << totalProducts << endl;
    }

    friend void comparePrice(const Product &p1, const Product &p2);
};

int Product::totalProducts = 0;

void comparePrice(const Product &p1, const Product &p2) {
    if (p1.productPrice > p2.productPrice)
        cout << "\n" << p1.productName << " is more expensive." << endl;
    else if (p1.productPrice < p2.productPrice)
        cout << "\n" << p2.productName << " is more expensive." << endl;
    else
        cout << "\nBoth products have the same price." << endl;
}

int main() {
    Product p1, p2;

    cout << "Enter details for Product 1:\n";
    p1.inputDetails();

    cout << "\nEnter details for Product 2:\n";
    p2.inputDetails();

    cout << "\nProduct Information:";
    p1.displayDetails();
    p2.displayDetails();

    Product::displayTotalProducts();

    comparePrice(p1, p2);

    return 0;
}
